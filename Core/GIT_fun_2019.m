...
clear z power bunching energy xrms yrms r_size Epart0 K_undulator
j = 1;
incK = 0;
incl = 0;
z(1) = 0;
...
if git_par.fieldcontourplot ==1 && ((itdp & islice==round(git_par.nslice/2)) || itdp==0)
    ...
    contourf(xgrid,xgrid,abs(fieldsq));
    cb=colorbar;
    ylabel(cb,'power');
end
...
if(period_tapering > 0) % Helical only
    ...
    aw0 = K_und(lambda_w);
elseif (period_tapering == 0 & quadrupole_lattice==1 & lat(1,j-1))
    if(helical)
